...
origin or supplier codes, marking several things unavailable — use
`manage-items.html`:
...
```
   ...
   - `supplierCode` — optional. A short opaque tag (e.g. "SUP-A") shown next
     to the SKU on the card. This is public — never put a supplier's actual
     name or contact info here. Keep the decoder (which code maps to which
     real supplier) only in your private supplier-map spreadsheet, never in
     this repo.
...
## Notes
- 65 product photos live in `images/`, cropped from your original photos with all
  reseller branding removed, compressed to ~25KB average. Each item in
  `items.json` references one via its `"image"` field. A few items that
  shared one source photo (e.g. GI Lintel's 3 sizes) point to the same file
